...
def viewnotes():
    if session.get('user'):
        ...
        return redirect(url_for('login')) 
@app.route('/nid/<uid>',methods=['GET','POST'])    
def vnid(uid):
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
    ...
    if session.get('user'):
        if request.method=='POST':
            ...
            for file in files: 
                ...
                username=session.get('user')
                cursor=mydb.cursor(buffered=True)
                cursor.execute('insert into files(fid,extention,filedata,added_by) values(uuid_to_bin(uuid()),%s,%s,%s)',[file_ext,file_data,username])
                mydb.commit()
            cursor.close()
...
def viewfid(uid):
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select extention,filedata from files where bin_to_uuid(fid)=%s',[uid])
